...
<script setup lang="ts">
  ...
  const total = ref(0);
  // 查询条件
  const query = ref<GetDictionaryItemPageParams>({
    ...defaultQuery,
  });
  /** 重置 */
  const handleReset = () => {
  ...
  /** 数据 */
  const tableData = ref<GetDictionaryItemAllResult[]>();
  ...
  const getTableData = async (type: boolean = false) => {
    ...
    const res = await getDictionaryItemPage(data);
  ...
  // 表单数据
  const form = ref<DictionaryItemCreate | DictionaryItemUpdate | DictionaryDetailItem>();
  const rules = ref({});
    ...
    try {
      ...
      const fns = {
        getDetail: async function () {
          let res = await getDictionaryItemDetail(id as string);
          form.value = res.data;
        },
        edit: async function () {
        ...
        delete: async function () {
          ElMessageBox.confirm("你确定要删除吗？", "删除字典值", {
            type: "error",
          }).then(async () => {
            let res = await deleteDictionaryItem(id as string);
            getTableData();
            ElMessage.success({
  ...
  const submit = () => {
    formRef.value?.validate(async (valid) => {
      ...
      try {
        ...
        if ("id" in form.value!) {
          // 编辑
          res = await updateDictionaryItem(form.value?.id, form.value);
        } else {
          // 新增
          res = await createDictionaryItem(form.value as DictionaryItemCreate);
        }
        dialogVisible.value = false;
        ElMessage.success({
          message: res?.message || "操作成功",
